...
wait_until_phone_is_connected_via_adb_and_online() {
  ...
  while true; do
    if [[ "$wait_sec" -gt "$max_wait_sec" ]]; then
      echo "Timeout error, device did not come back online via adb. Please retry."
      exit 5
    fi
    ...
    if ! command -v adb &>/dev/null || ! adb devices | grep -q "^[^*]*device$"; then
      ...
      wait_sec=$((wait_sec + 1))
      echo "Please wait until your phone ADB connection is online again [$wait_sec/$max_wait_sec])"
      adb devices
      sleep 1
    else
